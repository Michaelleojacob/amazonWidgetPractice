...
export function createElement(
  ...
  if (attributes instanceof Function)
    attributes = attributes(element, tagName, attributes, children);
        ...
        case "classList":
          ...
          value = value.reduce((classes, item) => {
            if (item instanceof Function)
              item = item(element, tagName, attributes, children);
            if (item instanceof Array)
              return classes.concat(
                ...item.filter((item) => item === 0 || !!item)
              );
            return classes.concat(
              ...(typeof item === "object"
  ...
  children
    .map((child) =>
      child instanceof Function
        ? child(element, tagName, attributes, children)
        : child
    )
  ...
  if (!tagName) {
    Object.defineProperty(element, "innerHTML", {
      get: () =>
        Array.from(
          element.childNodes,
          (item) => item.outerHTML || item.textContent
        ).join(""),
    });
  }
...
export function css(styles = {}) {
  const toDeclarations = ([key, value]) =>
    key.replace(/([A-Z])/g, "-$1").toLowerCase() + ": " + value;
  const stylesString = Object.entries(styles)
    .map(([selector, declarationsObject]) => {
      const declarations = Object.entries(declarationsObject)
        .map(toDeclarations)
        .join("; ");
